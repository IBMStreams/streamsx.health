...
/**
 * Per-cache component that manages cache event listener registrations, and
 * provides event delivery based on desired firing mode for specified event
 * types.
 * <p>
 * Use of this class is linked to having cache events on a
 * {@link org.ehcache.UserManagedCache user managed cache}.
 * <p>
 * <em>Note on event ordering guarantees:</em> Events are received and
 * transmitted to register listeners through the registration of a
 * {@link StoreEventListener} on the linked {@link StoreEventSource} which is
...
public class OSObjectCacheEventDispatcher<K, V> implements CacheEventDispatcher<K, V> {
	...
	/**
	 * Creates a new {@link CacheEventDispatcher} instance that will use the
	 * provided {@link ExecutorService} to handle events firing.
	 *
	 * @param unOrderedExecutor
	 *            the executor service used when ordering is not required
	 * @param orderedExecutor
	 *            the executor service used when ordering is required
	 */
	...
	public void registerCacheEventListener(CacheEventListener<? super K, ? super V> listener, EventOrdering ordering,
			EventFiring firing, EnumSet<EventType> forEventTypes) {
		...
		// we have to avoid the submit if no listeners are registered for this
		// event.
	...
	/**
	 * Synchronized to make sure listener addition is atomic in order to prevent
	 * having the same listener registered under multiple configurations
	 *
	 * @param wrapper
	...
	/**
	 * OSObjectEventDispatchTask Synchronized to make sure listener removal is
	 * atomic
	 *
	 * @param wrapper
	 *            the listener wrapper to unregister
	 * @param listenersList
	 *            the listener list to remove from
	 */
	private synchronized boolean removeWrapperFromList(EventListenerWrapper wrapper,
			List<EventListenerWrapper<K, V>> listenersList) {
	...
	public List<CacheConfigurationChangeListener> getConfigurationChangeListeners() {
		...
		configurationChangeListenerList.add(event -> {
			if (event.getProperty().equals(CacheConfigurationProperty.ADD_LISTENER)) {
				...
				CacheEventListener<? super K, ? super V> oldListener = (CacheEventListener<? super K, ? super V>) event
						.getOldValue();
	...
	private final class StoreListener implements StoreEventListener<K, V> {
		...
		public void onEvent(StoreEvent<K, V> event) {
			...
			case CREATED:
				OSObjectCacheEventDispatcher.this
						.onEvent(CacheEvents.creation(event.getKey(), event.getNewValue(), listenerSource));
				break;
			case UPDATED:
				if (updateFireRequired)
					OSObjectCacheEventDispatcher.this.onEvent(CacheEvents.update(event.getKey(), event.getOldValue(),
							event.getNewValue(), listenerSource));
				break;
			case REMOVED:
				OSObjectCacheEventDispatcher.this
						.onEvent(CacheEvents.removal(event.getKey(), event.getOldValue(), listenerSource));
				break;
			case EXPIRED:
				OSObjectCacheEventDispatcher.this
						.onEvent(CacheEvents.expiry(event.getKey(), event.getOldValue(), listenerSource));
				break;
			case EVICTED:
				OSObjectCacheEventDispatcher.this
						.onEvent(CacheEvents.eviction(event.getKey(), event.getOldValue(), listenerSource));
				break;
			default:
